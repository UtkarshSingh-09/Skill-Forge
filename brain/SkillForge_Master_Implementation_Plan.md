...
| File name | Build | Duration | How to shoot (vary these WHILE filming) |
|---|---|---|---|
| `sim1_led_blink.mp4` | LED blink circuit | **~100 s** | Orbit 90°→30°; change lighting mid-clip (ceiling → side lamp → dim); include ~15s with a hand adjusting a wire |
| `sim2_alternate_blink.mp4` | 2-LED alternate | **~100 s** | Same variety: top-down, 45°, low angle; bright + shadowed + daylight |
| `sim3_binary_count.mp4` | 2-LED binary | **~100 s** | Same variety |
| `sim4_morse.mp4` | Morse LED | **~100 s** | Same variety |
| `board_empty.mp4` | **Empty + half-populated** board (no fixed circuit) | **~80 s** | Multiple angles/lighting — teaches the board-pose model the bare grid |
